...
void kanGPU(tensor ***x, tensor ***fourier_coeffs, tensor **res){
	constexpr int blocks = 10000;
	constexpr int threads = 1024;
	constexpr int totalThreads = blocks*threads;

	cudaMalloc((void ****)&x, totalThreads *sizeof(float));
	cudaMalloc((void ****)&fourier_coeffs, totalThreads *sizeof(float));

    cudaMemset(y, 0, totalThreads *sizeof(float));

    kanKernel<<<>>>(x,fourier_coeffs,y);

	cudaMemcpy(res, devResults, totalThreads * sizeof(float), cudaMemcpyDeviceToHost);
